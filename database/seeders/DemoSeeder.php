...
class DemoSeeder extends Seeder
{
    public function run(): void
    {
        ...
        $user=User::firstOrCreate(['login'=>'cliente@alugapro.local'],['name'=>'Ana Souza','email'=>'cliente@alugapro.local','cpf'=>'123.456.789-00','phone'=>'(81) 98888-0102','role'=>'client','active'=>true,'password'=>'Cliente@2026']);
        $client=Client::firstOrCreate(['cpf'=>'123.456.789-00'],['user_id'=>$user->id,'name'=>$user->name,'phone'=>$user->phone,'email'=>$user->email,'family_income'=>6500,'status'=>'active']);
        ...
        $rent=Charge::firstOrCreate(['lease_id'=>$lease->id,'type'=>'rent','reference_month'=>now()->startOfMonth()],['client_id'=>$client->id,'due_date'=>now()->startOfMonth()->day(10),'amount'=>1600,'status'=>now()->day>10?'paid':'open','paid_at'=>now()->day>10?now()->startOfMonth()->day(10):null,'description'=>'Aluguel do mês']);
        $solarCharge=Charge::firstOrCreate(['lease_id'=>$lease->id,'type'=>'solar','reference_month'=>now()->startOfMonth()],['client_id'=>$client->id,'due_date'=>now()->startOfMonth()->day(10),'amount'=>111.15,'status'=>'open','description'=>'Energia solar: 117 kWh']);
