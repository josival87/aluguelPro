...
Route::middleware(['auth', 'role:admin,manager'])->prefix('admin')->name('admin.')->group(function () {
    ...
    Route::delete('/usuarios/{user}', [UserController::class, 'destroy'])->middleware('role:admin')->name('users.destroy');
    Route::resource('grupos', GroupController::class)->parameters(['grupos' => 'group'])->names('groups')->except(['show', 'destroy']);
    Route::delete('/grupos/{group}', [GroupController::class, 'destroy'])->middleware('role:admin')->name('groups.destroy');
    Route::resource('clientes', ClientController::class)->parameters(['clientes' => 'client'])->names('clients')->except('destroy');
    Route::delete('/clientes/{client}', [ClientController::class, 'destroy'])->middleware('role:admin')->name('clients.destroy');
    Route::get('/clientes/{client}/documentos/{document}', [ClientDocumentController::class, 'show'])->name('clients.documents.show');
    Route::resource('imoveis', PropertyController::class)->parameters(['imoveis' => 'property'])->names('properties')->except('destroy');
    Route::delete('/imoveis/{property}', [PropertyController::class, 'destroy'])->middleware('role:admin')->name('properties.destroy');
    Route::post('/imoveis/{property}/midias', [AdminPropertyMediaController::class, 'store'])->name('properties.media.store');
    Route::delete('/imoveis/{property}/midias/{propertyMedia}', [AdminPropertyMediaController::class, 'destroy'])->middleware('role:admin')->name('properties.media.destroy');
    Route::resource('contratos', AdminContractController::class)->parameters(['contratos' => 'contract'])->names('contracts')->except('destroy');
    Route::delete('/contratos/{contract}', [AdminContractController::class, 'destroy'])->middleware('role:admin')->name('contracts.destroy');
    Route::resource('alugueis', LeaseController::class)->parameters(['alugueis' => 'lease'])->names('leases')->except('destroy');
    Route::delete('/alugueis/{lease}', [LeaseController::class, 'destroy'])->middleware('role:admin')->name('leases.destroy');
    ...
    Route::delete('/caracteristicas/{feature}', [FeatureController::class, 'destroy'])->middleware('role:admin')->name('features.destroy');
