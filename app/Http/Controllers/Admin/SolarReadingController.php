...
class SolarReadingController extends Controller
{
    ...
    {
        $leases=Lease::with('property','solarConfig.readings')->where('status','active')->where('has_solar_energy',true)->orderByDesc('id')->get();
    ...
    public function analyze(Request $request,MeterOcrService $ocr)
    {
        ...
        try{return response()->json($ocr->read($request->file('photo')));}
        catch(Throwable $e){return response()->json(['reading'=>null,'confidence'=>0,'candidates'=>[],'requires_confirmation'=>true,'message'=>$e->getMessage()],422);}
    }
    public function store(Request $request)
    {
        ...
        $lease=Lease::with('solarConfig.readings')->findOrFail($data['lease_id']);abort_unless($lease->has_solar_energy && $lease->solarConfig,422,'Aluguel sem configuração solar.');
        ...
        $current=(float)$data['meter_reading'];abort_if($current<$previous,422,"A leitura atual não pode ser menor que {$previous} kWh.");
        $consumption=round($current-$previous,3);$amount=round($consumption*(float)$lease->solarConfig->price_per_kwh,2);$file=$request->file('photo');
