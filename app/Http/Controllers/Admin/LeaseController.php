...
class LeaseController extends Controller
{
    ...
    public function create() { return view('admin.leases.form',['lease'=>new Lease,'clients'=>Client::orderBy('name')->get(),'properties'=>Property::whereIn('status',['available','paused'])->orderBy('title')->get()]); }
    public function store(Request $request, ContractService $contracts)
    {
        ...
        $lease=DB::transaction(function() use($request,$data,$contracts){
            ...
            if($lease->status==='active'){$lease->property()->update(['status'=>'rented']);}
            return $lease;
        });
    ...
    public function edit(Lease $lease) { return view('admin.leases.form',['lease'=>$lease->load('solarConfig'),'clients'=>Client::orderBy('name')->get(),'properties'=>Property::where(fn($q)=>$q->whereIn('status',['available','paused'])->orWhereKey($lease->property_id))->orderBy('title')->get()]); }
    public function update(Request $request, Lease $lease, ContractService $contracts)
    {
        ...
        DB::transaction(function() use($request,$data,$lease,$contracts){
            $lease->update($data);$this->solar($request,$lease);
            if(!$lease->contract)$contracts->generate($lease);
            if($lease->status==='active')$lease->property()->update(['status'=>'rented']);
            if(in_array($lease->status,['cancelled','finished'],true))$lease->property()->update(['status'=>'available']);
        });
        return redirect()->route('admin.leases.show',$lease)->with('success','Aluguel atualizado.');
    }
    public function destroy(Lease $lease) { abort_if($lease->charges()->exists(),422,'Aluguel possui cobranças. Cancele-o em vez de excluir.'); $lease->delete(); return redirect()->route('admin.leases.index')->with('success','Aluguel excluído.'); }
    private function validated(Request $request, ?Lease $lease=null): array
    {
        $data=$request->validate([
            ...
            'rent_amount'=>['required','numeric','min:0'],'status'=>['required',Rule::in(['awaiting_completion','awaiting_signatures','active','finished','cancelled'])],
        ...
        if ($data['status'] === 'active' && $lease?->contract?->status !== 'signed') {
            throw ValidationException::withMessages(['status' => 'O aluguel só pode ser ativado após as duas assinaturas do contrato.']);
        }
        if(!empty($data['start_date']) && empty($data['end_date']))$data['end_date']=Carbon::parse($data['start_date'])->addMonths((int)$data['contract_months'])->toDateString();
        return $data;
    }
    private function solar(Request $request, Lease $lease): void
    {
        if($lease->has_solar_energy)$lease->solarConfig()->updateOrCreate([],['initial_reading'=>$request->input('initial_reading'),'price_per_kwh'=>$request->input('price_per_kwh')]);
        elseif($lease->solarConfig && !$lease->solarConfig->readings()->exists())$lease->solarConfig()->delete();
    }
}
