...
{
    public function __construct(private readonly MiaIntegrationService $mia) {}
    ...
    public function settle(Charge|int $charge, string $paymentMethod = 'manual'): array
    {
        ...
        $result = DB::transaction(function () use ($chargeId, $paymentMethod) {
            $locked = Charge::query()->lockForUpdate()->findOrFail($chargeId);
            if ($locked->status === 'paid') {
                return ['charge' => $locked, 'changed' => false, 'mia_receipt' => null];
            }
            ...
            return [
                'charge' => $locked,
                'changed' => true,
                'mia_receipt' => $this->mia->record($locked),
            ];
        });

        $this->mia->dispatch($result['mia_receipt']);
        unset($result['mia_receipt']);
