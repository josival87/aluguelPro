...
@section('content')
<div class="page-head"><div><a style="color:var(--blue);font-weight:700" href="{{ route('admin.leases.index') }}">← Aluguéis</a><h1 style="margin-top:8px">Aluguel #{{ $lease->id }}</h1><p>{{ $lease->property->title }} · {{ $lease->client->name }}</p></div><div class="head-actions"><x-status :value="$lease->status"/><a class="btn btn-outline" href="{{ route('admin.leases.edit',$lease) }}"><x-icon name="edit"/><span>Finalizar / editar</span></a>@if(auth()->user()->isAdministrator() && $lease->status === 'closed')<form method="post" action="{{ route('admin.leases.destroy', $lease) }}" data-confirm="Excluir este aluguel encerrado e todos os seus dados vinculados, como cobranças e documentos? Esta ação não pode ser desfeita.">@csrf @method('DELETE')<button class="btn btn-danger" type="submit"><x-icon name="trash"/> Excluir</button></form>@endif</div></div>
